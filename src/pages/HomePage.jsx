...
const HomePage = () => {
  ...
  const allProducts = currentCategory || searchQuery ? getFilteredProducts() : getFeaturedProducts();
  const {
    filteredProducts,
  ...
  useEffect(() => {
    ...
    if (!loading && getFeaturedProducts().length === 0 && !error) {
      refreshData();
    }
  }, []);
  ...
  const productsToShow = currentCategory || searchQuery 
    ? getFilteredProducts() 
    : getFeaturedProducts();
